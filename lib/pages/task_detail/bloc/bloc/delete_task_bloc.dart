import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:test_task_tracker/data/repositories/task_repository.dart';
import 'package:test_task_tracker/pages/task_detail/bloc/event/delete_task_event.dart';
import 'package:test_task_tracker/pages/task_detail/bloc/state/delete_task_state.dart';

class DeleteTaskBloc extends Bloc<DeleteTaskEvent, DeleteTaskState> {
  DeleteTaskBloc({required TaskRepository repository})
    : _repository = repository,
      super(const DeleteTaskState()) {
    on<DeleteTaskIdleEvent>(_onIdle);
    on<DeleteTaskSubmitEvent>(_onSubmit);
  }

  final TaskRepository _repository;

  void _onIdle(DeleteTaskIdleEvent event, Emitter<DeleteTaskState> emit) {
    emit(state.copyWith(status: DeleteTaskStatus.idle, message: ''));
  }

  Future<void> _onSubmit(
    DeleteTaskSubmitEvent event,
    Emitter<DeleteTaskState> emit,
  ) async {
    emit(state.copyWith(status: DeleteTaskStatus.loading, message: ''));
    try {
      await _repository.deleteTask(event.id);
      emit(
        state.copyWith(
          status: DeleteTaskStatus.loaded,
          message: 'Task berhasil dihapus',
        ),
      );
    } catch (_) {
      emit(
        state.copyWith(
          status: DeleteTaskStatus.error,
          message: 'Gagal menghapus task',
        ),
      );
    }
  }
}
